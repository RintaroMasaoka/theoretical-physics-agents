...
This project uses a template-based generation system. Prompt files are generated from templates — editing generated files directly is incorrect because `configure.mjs` overwrites them on each run.

| Layer | Path | Role |
|---|---|---|
| **Template (source of truth)** | `templates/**/*.src.md` | Edit these |
| **Generated (do not edit)** | `.claude/agents/*.md`, `.claude/skills/*/SKILL.md`, `.claude/CLAUDE.md`, `.claude/common.md` | Overwritten by `node configure.mjs` |
...
- Preserve `{{ placeholder }}` syntax — these are config variables resolved by `configure.mjs`

### 2. Regenerate

After writing the `.src.md` file, run `node configure.mjs` to regenerate all runtime files. Verify the generated output matches expectations.
...
Report the verification agent's findings to the user, and fix any valid findings. If fixes are made, run `node configure.mjs` again.
...
2. **Upstream sync**: Run `bash sync.sh push --yes` to push framework changes (templates, config, AGENTS.md, configure.mjs) to the upstream remote — the public repo that other projects pull from. This keeps all downstream projects in sync with the improvement

...
- **Do not conduct new research.** If a research gap is discovered during writing, record it in `meetings/agenda.md` and leave it to `/run`. Do not launch researcher or scout yourself
...
```
plan.md                   # Read-only (item tree, Story Arc, strategy)
items/                    # Read-only (item context, kind, status)
...
| Agent | Role |
|---|---|
| **outliner** | Design the overall paper structure from the Story Arc and item tree |
...
2. Read `plan.md` (Story Arc with item tree)
3. Read `items/*.md` (all items files — understand kind, status, context)
...
- Check the status of core items in the Story Arc. If too many are open/active, writing will require major rewrites later. In that case, write "Writing deferred due to insufficient research. The following items need resolution first: ..." in `meetings/agenda.md`, report to the user in the final report, and exit
...
Phases are guidelines; judge flexibly according to the situation. For example, if a review reveals insufficient content, record the research gap in `meetings/agenda.md`, put that section on hold, and proceed with other sections.
...
- **outliner**: (no additional data — reads plan.md, items/, notes/index.md on its own)
- **writer**: `Assigned section: #{N} {title} (slug: {slug})` / `Related Items: {ids}` / `Evidence: {paths}` / For revisions: `Review report: {path}`
- **reviewer**: `Target section: manuscript/sections/{N}_{slug}.md` / `Related Items: {ids}`
...
- **Research gap discovery**: If writer or reviewer reports insufficient evidence, record specifically in `meetings/agenda.md` (which item is lacking what)
- **Update TodoWrite**

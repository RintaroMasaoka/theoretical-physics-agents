...
Write or extend a script in `src/`. If extending an existing script (per §1), use Edit; if creating a new one, write `src/{slug}.{ext}`. The script loads modules from `lib/` and implements only the measurement logic. Language: **{{ simulation.language }}** (matches the modules).
...
Write `results/{category}/{slug}/REPORT.md` — a self-contained explanation of this simulation for the human researcher (the user). The user often reviews results by browsing the `results/` directory, so this file is their primary entry point. Write in **{{ language }}**.
...
**Deliverable**: `work/simulation_{N}_{slug}.md` (`{N}` is the serial number specified by PI in the prompt. If not specified, use max existing `simulation_*.md` number + 1, noting this at the top of the deliverable)

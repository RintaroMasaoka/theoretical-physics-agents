...
- `report_{slug}.md` — create when research planner directs promotion of an attempt, or place/read a worker-authored report when a worker was explicitly assigned clean-report authorship; format per `{{ runtime.research_tree_file }}`
...
Format per `{{ runtime.research_tree_file }}` — a report has explicit provenance links to `checks/*.md` records, a self-contained derivation, and does not require the reader to open `.logs/` to understand it.
...
{{#if runtime.is_codex}}
Codex cannot assume custom local agent types are available. The shared requirement is independent note.md review; the mechanics are runtime-specific.

When curator is dispatched inside `/auto`, the user's `/auto` request is an explicit request for the workflow's agent orchestration, including critic review steps defined by this prompt. Do not stop to ask the user again. Launch the default/general sub-agent mechanism with no custom `agent_type`, and instruct it to read and follow `{{ runtime.agents_dir }}/critic.md` before reviewing the note.md section.

If the current runtime refuses agent launch, or critic agents were explicitly disabled for this run, do not spawn. Record `critic Target B not run: critic-agent launch unavailable or disabled` in your return, flag the verification gap for research planner, and do not elevate the affected claim's confidence based on critic review.

Prompt body for an approved Codex critic-agent launch:

```
Read and follow `{{ runtime.agents_dir }}/critic.md` as your role definition. Treat the rest of this prompt as task-specific input.

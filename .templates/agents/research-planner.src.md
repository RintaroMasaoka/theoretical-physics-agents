...
4. `{{ runtime.naming_file }}` — read when your focus text, child names, worker dispatches, or tree directives give a short expression handle shape: repeated shorthand, child/focus label, dispatch handle, tree-directive term, or reuse of an unlicensed phrase from earlier context
...
### Naming Decisions
- {only when this focus, a child name, a worker dispatch, or a tree directive gives a short expression handle shape or reuses an unlicensed earlier phrase as a handle; use `{{ runtime.naming_file }}`}
- (may be empty)

...
Each node has a `kind` and `status` in its **state.md** frontmatter (not note.md). Node status is set by curator, based on research planner's Tree Directives and evidence accumulated in state.md (see `{{ runtime.agents_dir }}/curator.md` and `{{ runtime.agents_dir }}/research-planner.md`).

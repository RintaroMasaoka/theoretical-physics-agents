...
| `concepts/{term}.md` | Naming / reader bridge | **Reusable-name license plus reader bridge.** Explains reusable handles and records what later readers may infer, carry, merge, or not merge from the name. Not fact authority, not a convention ledger, not workflow state. See `{{ runtime.naming_file }}` |
...
`concepts/` contains reusable-name licenses and reader bridges. A reusable name is a compact handle that has already been given handle shape in project memory; the concept note records the name's expansion, grounding, stability, carry scope, claim permission, merge boundaries, and reader action per `{{ runtime.naming_file }}`. The prose body explains the name for a neighbouring-field researcher.

...
**Principle: Ask questions first, then load data only as needed after responses.** Meetings are real-time dialogue, and response delays interrupt the user's thinking. Use {{ runtime.tool_ask_user_question }} for questions (text output questions risk missed responses).
...
{research tree label in {{ language }}}:
  ...
  {open label in {{ language }}}: {N} nodes
{key achievements since last meeting label in {{ language }}}: [summary]
```

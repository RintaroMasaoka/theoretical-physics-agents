...
File formats (note.md, state.md) are defined in `{{ runtime.research_tree_file }}`. The content emerges from the research discussion — there are no prescribed section names. AI drafts the prose and confirms with the user.
...
Capture ISO timestamp at session start via `{{ runtime.tool_shell }}("date '+%Y-%m-%dT%H:%M'")` for traceability markers. Obtain the launch log path via `bash .scripts/log-path.sh launch` — the script returns a timestamped path of the form `.logs/{YYMMDD_HHMM}_launch.md`; write the launch log to that path.

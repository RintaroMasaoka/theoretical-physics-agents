...
3. **Assemble wrap-up input** at `logs/_DRAFT_wrap-up-input.md`. PI writes the substantive content here; the `session-wrap-up` agent distributes it to the right files, cleans up the beacon, and commits/pushes.
   ...
   **Templates referenced above.**
   ...
   The `Session Log` section becomes `logs/{timestamp}_run.md` (permanent record, never overwrite). Use the timestamp captured at Session Start; a system hook renames `_DRAFT_run.md` with the correct timestamp on write.

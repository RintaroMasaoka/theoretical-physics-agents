...
{{#if runtime.is_codex}}
{{ runtime.tool_agent }}(prompt="""
Read and follow `{{ runtime.agents_dir }}/guide-writer.md` as your role definition. Treat the rest of this prompt as task-specific input.
...
{{else}}
{{ runtime.tool_agent }}({{ runtime.tool_agent_type_field }}="guide-writer", prompt="""
## Task
Session-end guide sweep. For each target below, read the durable node surfaces and update guide.md only if the human oversight entrypoint is missing or stale. Do not decide research direction, claim admission, verification status, or graph placement.
{{/if}}

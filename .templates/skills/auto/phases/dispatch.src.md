...
{{else}}
Invoke it with `{{ runtime.tool_agent_type_field }}="{name}"`. The scheduler's prompt contains only task-specific information — the agent's own definition carries the reading protocol, deliverable format, and operating rules.
{{/if}}

...
File formats (findings.md, guide.md, plan.md, backlog.md, state.md) are defined in `{{ runtime.research_tree_file }}`. This section covers only files specific to `/auto` operations.
...
Compact, but always in the structured format required by `{{ runtime.agents_dir }}/research-planner.md` § `research/focus.md` Format. No frontmatter. Overwritten at each session end (see `session-lifecycle.md` for the handoff). Points to the current focus node and carries short-term context only.

...
| **Generated (do not edit)** | `{{ runtime.agents_dir }}/*.md`, `{{ runtime.skills_dir }}/*/SKILL.md`, `{{ runtime.instruction_file }}`, `{{ runtime.common_file }}` | Overwritten by `generate-runtime.mjs` |
...
**Reflect and confirm.** Apply valid blind-review findings and fit-check findings, then re-run `generate-runtime.mjs` if changes were made. Then present the final change to the user via {{ runtime.tool_ask_user_question }} for approval.

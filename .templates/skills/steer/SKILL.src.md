...
- **Write all prose in {{ language }}.** Applies to conversational text, questions, `.logs/`, `research/focus.md`, curator tree writes, worker deliverables, and commit messages. Technical terms, proper nouns, LaTeX mathematics, file/folder slugs, frontmatter keys, and documented structural headings may stay in English.
...
|---|---|---|
| `{{ runtime.skills_dir }}/auto/phases/dispatch.md` | After the Steering Gate, when launching workers and critic | Pattern A / B launch methods, prompt template, Provisional Artifact Review, Durable Surface Review, per-agent dynamic data |

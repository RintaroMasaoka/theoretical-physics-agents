...
Run the `/auto` Session Start gates and resume handling from `{{ runtime.skills_dir }}/auto/SKILL.md` / `auto/phases/session-lifecycle.md`, with these adjustments:
...
Read and follow `{{ runtime.agents_dir }}/research-plan-presenter.md` as your role definition. Treat the rest of this prompt as task-specific input.
...
{{else}}
{{ runtime.tool_agent }}({{ runtime.tool_agent_type_field }}="research-plan-presenter", prompt="""
## Task
Present the proposed `research/focus.md` plan as a self-contained human checkpoint. Do not change `research/focus.md` or any research-tree file. Return the checkpoint inline.
{{/if}}
...
Present the checkpoint compactly and ask one approval question with `{{ runtime.tool_ask_user_question }}`:

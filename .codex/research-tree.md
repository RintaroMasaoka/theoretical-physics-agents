...
| `.todo.md` | Workbench (hidden) | **Node-local tactical backlog. Curator-maintained.** Short-lived next actions, reminders, and coordination notes for active work at this node. It is not user-facing, not evidence, not strategy, and not a Source of Truth; it exists to keep tactical residue out of context memory without polluting `plan.md` or `.log.md`. Create, rewrite, or delete it when a node has enough local unfinished work that keeping it only in `research/focus.md` would be fragile |
...
**Node-root prohibition.** Never place source files (`.py`, `.jl`, …) directly in the node folder. Node roots are reserved for narrative files (`.log.md`, `plan.md`, `.todo.md`, `note.md`, `report_*.md`, `story.md`, `principles.md`, `conventions.md`) plus named artifact directories such as `checks/`, `src/`, `data/`, and `images/`. Loose scripts in node roots break the tree's legibility and bypass the `src/` reuse rule, so creating `src/` (even for a single script) is mandatory.
...
| **Research tree — todo** | `research/**/.todo.md` | Write (curator) | Hidden short-term checklist for active node-local work; no claims, evidence, or durable strategy |
...
**Tree navigation**: `ls research/{path}/` to see children (subfolders). Read `note.md` for verified knowledge (SoT), `report_*.md` for verified analyses, `checks/` for node-local verification records, `plan.md` for strategy and decomposition, `.log.md` for current research state and evidence, `story.md` for narrative structure, `principles.md` for constraints, and `conventions.md` for notation / convention choices. Read `.todo.md` only when you are operating on that node and need its current checklist; do not treat it as a source for claims, strategy, or research state.

Each node has a `kind` and `status` in its **.log.md** frontmatter (not note.md). Node status is set by curator, based on physicist's Tree Directives and evidence accumulated in .log.md (see `.codex/agents/curator.md` and `.codex/agents/physicist.md`).

- Writes to the research tree are split by agent: **physicist** writes only `research/focus.md` (cursor + directives + worker dispatch plan); **curator** writes everything else in the tree — node folders, `.log.md`, `plan.md`, `.todo.md`, `dead_ends.md`, `asides.md`, `report_*.md`, `checks/`, `note.md`, `story.md`, `principles.md`, and `conventions.md` — executing physicist's directives plus its own default operating rules (including splitting overloaded nodes when the evidence record has outgrown the parent scope; see § note.md — Ownership and curator.md). Curator may also move superseded scripts from `src/` to `src/archive/` when physicist directs archival, because the move and its prose record must stay together. **critic** may write only curator-dispatched Target B review files under `research/**/checks/`; simulator writes under `data/`, `images/`, and `src/`; engine-builder writes under `lib/`. No other agent writes to the tree. If physicist notices a tactical item worth preserving in `.todo.md`, it should state that in `research/focus.md`; if any other agent notices one, it should state it in its deliverable. Curator decides whether it belongs in `.todo.md`

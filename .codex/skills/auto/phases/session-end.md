...
This phase file is kept only so old links do not break. The active session-end procedure lives in `session-lifecycle.md` § Session End, and the research planner-owned wrap-up-input format lives in `.codex/agents/research-planner.md` § Session-End Mode.

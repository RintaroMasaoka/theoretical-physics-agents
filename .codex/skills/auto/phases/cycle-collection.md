...
- *Report-scope trigger*: when a deliverable is ready for promotion into `report_{slug}.md`, check whether the report's topic matches only one of multiple angles listed in the parent's map.md / plan.md / Current Board. If it matches only one angle, create a child node for that scope and place the report there instead of at the parent — reports belong at the node whose scope they match, not at the nearest convenient ancestor
...
For creation mechanics and structural closure, see research-planner.md, curator.md, and `architecture.md`. **After any node creation, curator updates the parent's map.md** to record the new child's parent role, status, implication, and reopen condition; it updates plan.md only when the strategic decomposition changed. If research planner creates the minimal child, it may set the cursor or worker target to that child in the same cycle, but it must leave parent integration, evidence copy, link hygiene, and lifecycle cleanup to curator via a structural-closure directive.

## Update plan.md (when strategy changes)

If results change the approach — new decomposition, reprioritized children, revised strategy — research planner states the needed change as a Tree Directive or curator applies it from structural-maintenance judgment. Curator updates map.md for child routing and plan.md for strategy. map.md reflects how to read children; plan.md reflects how to proceed; state.md records what happened and what is known.
...
note.md is the draft fact layer — canonical rule in `.codex/research-tree.md` § note.md. It is not current state, not a report index, and not final manuscript authority.
...
2. If yes, read the parent's state.md, map.md, and plan.md. If Current Board, status, map.md, or plan.md needs revision, express that as a Tree Directive; curator executes.
...
- **Check figures**: View PNGs via read file tool, visually confirm trends and agreement

...
The tree must grow with the research, not stay frozen in the shape `/meeting` or `/launch` first gave it. **Node creation is not gated by meeting approval** — `directives.md` is the only meeting-exclusive structure (cf. the file-role table in `architecture.md`). Research planner requests splits when direction-setting reveals the need; curator may also create splits from structural-maintenance judgment when the evidence record already shows the parent has become overloaded. A newly created node that turns out to be a misread can be closed or merged on a later cycle, which is cheaper than leaving the tree stale.
...
For creation mechanics (mkdir + state.md initialisation), see curator.md and `architecture.md`. **After any node creation, curator updates the parent's plan.md** to record the new child's role and the decomposition rationale; skipping plan.md silently decouples the tree from the plan document. If a split is requested by research planner, keep the cursor at the parent until curator has created the child; the next research planner dispatch can descend one edge.

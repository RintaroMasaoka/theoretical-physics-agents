...
- **curator**: Pass a concrete checklist so curator can judge without re-scanning the whole tree. PI's role is to enumerate raw **candidates**; filtering/judgment is curator's (per curator's own default-create rule in `.codex/agents/curator.md` § note.md Maintenance):

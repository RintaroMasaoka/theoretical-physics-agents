...
Codex does not support custom named `agent_type` values for local prompt files. Dispatch an ordinary `spawn_agent` and make the first line of the prompt mandatory: `Read and follow .codex/agents/{agent}.md as your role definition. Treat the rest of this prompt as task-specific input.` The scheduler then writes only task-specific information below that line.


```

Read and follow `.codex/agents/{agent}.md` as your role definition. Treat the rest of this prompt as task-specific input.

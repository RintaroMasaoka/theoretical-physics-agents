...
This phase file is a reference that PI Reads during `/run` at the beginning of every session. It defines the resume-check logic, the ancestor-chain read sequence, the unread-paper rule, and the initial sanity check. Execute each step in order.

---

## Initial Check (before step 0)

These guards must fire *before* the step sequence begins, because later steps Read files whose absence is the trigger:

- `research/log.md` does not exist → Display "Please set a theme via `/launch`" and stop (no further steps run)
- `research/focus.md` does not exist → Read the full tree. Create research/focus.md pointing to the first active node, then proceed to step 0
- `concepts/` does not exist → Create `concepts/` and write initial concept notes for core terms, then proceed to step 0
...
For papers marked `unread` in reading_list.md, PI must not describe their content, claims, methods, or results. Only the arXiv ID, title, authors, and a one-sentence abstract summary may be stated (see `.codex/common.md` verification procedures).

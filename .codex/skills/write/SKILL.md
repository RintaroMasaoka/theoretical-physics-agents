...
- **Internal/session prose uses japanese.** This includes `.logs/{timestamp}_write.md`, `.logs/last_write_session.md`, project-root `agenda.md`, and the final report. Reason: `japanese` is the project's working communication language.
...
- `request_user_input` is prohibited. Users are often away, and asking questions interrupts the session and wastes time
...
**Launch method:** Use the same Pattern A / Pattern B mechanics defined in `.codex/skills/auto/phases/dispatch.md`. Load that file when launching or waiting on subagents; reuse only its orchestration mechanics, not `/auto`'s research-cycle policy.

**Prompt template:**

Each agent is defined in `.codex/agents/{agent}.md`.

Codex does not support custom named `agent_type` values for local prompt files. Dispatch an ordinary `spawn_agent` and make the first line of the prompt mandatory: `Read and follow .codex/agents/{agent}.md as your role definition. Treat the rest of this prompt as task-specific input.` Write only task-specific information below that line:
...
2. **Write session log**: obtain the path via `bash .scripts/log-path.sh write` (returns `.logs/{YYMMDD_HHMM}_write.md`); write to that path (permanent record — never overwrite):
   ```markdown
   # {write-session title in japanese} YYYY-MM-DD HH:MM

   ## {accomplished heading in japanese}
   - {sections drafted, reviewed, finalized}

   ## {section-status heading in japanese}
   - {per-section status: not started / drafted / review PASS / FAIL}

   ## {issues heading in japanese}
   - {research gaps reported, blockers}

   ## {manuscript-freeze heading in japanese}

...
Use this checklist as an internal scan, not as the output format. Do not copy the checklist into the deliverable. Check each item mentally as a way to provoke opposition. You may raise multiple challenges, one challenge, or a tentative weak challenge. What you must not do is manufacture coverage: no separate objection is needed for a checklist item unless it genuinely creates pressure on the current direction.
...
- [ ] **Authority challenge** — Is a premise protected because it came from the user, literature language, or a previous agent rather than because the current evidence supports it? LLMs tend to over-respect inherited wording.
...
## Challenges
- **{short name of the objection or doubt}** — {the challenge itself. Then state what in the local board made you raise it. If the challenge is weak or tentative, say why rather than hiding the weakness.}
...
If `research/focus.md` is missing, still write the challenge with available root context and set unavailable sections to `(focus.md missing — research planner must initialise at research/ root)`.

...
1. `.codex/common.md`
2. `.codex/research-tree.md` — canonical specification for every file role, note.md rules, provenance taxonomy
...
- `note.md` — create, update, retract in the current runtime as the fact-maintenance transaction closer. Derivation-bearing draft fact layer per `.codex/research-tree.md` § note.md
- `dead_ends.md` — append when a closed node carries lessons; append when a retraction records a falsified claim's lesson
- `reports/{slug}.md` — create or place only through a promotion transaction: planner may direct the promotion, a worker may have been explicitly assigned clean-report authorship, or your maintenance pass may find an already-admitted bounded analysis whose parent node and surface fit are mechanically clear. In all cases, close review/provenance before the tree treats the report as verified support; format per `.codex/research-tree.md`
...
Format per `.codex/research-tree.md` — a report has explicit provenance links to `checks/*.md` records for principal claims, a self-contained derivation or computation narrative, and does not require the reader to open `.logs/` to understand it.
...
Audience — the context-free reader. Canonical definition: `.codex/research-tree.md` § note.md → Audience. Operational summary: the reader has only this note.md plus the files its Markdown links resolve to. No `.logs/`, no `plan.md`, no `state.md`, no project-internal vocabulary. A note.md that reads fluently to someone who just reread the logs but is opaque to anyone else fails.
...
Canonical rationale: `.codex/research-tree.md` § Durable Surface Review on note.md and reports, and § reports/{slug}.md. Operational rules:

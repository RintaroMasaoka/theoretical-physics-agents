...
- **Check existing scripts and data** before designing anything new — duplication comes from skipping this step:
  1. Search the target node's `_materials/src/` and ancestor nodes' `_materials/src/` for scripts measuring the same observable — under the canonical lowest-common-ancestor rule (see `.codex/research-tree.md` § `_materials/`), a shared measurement script may live several levels up. Prefer extending an existing script over creating a new one
...
Write or extend a script under the correct `_materials/src/` per the canonical placement rule (usually the target node; higher up if the script is shared — see `.codex/research-tree.md` § `_materials/`). If extending an existing script (per §1), use Edit; if creating a new one, write `{slug}.{ext}` in that `_materials/src/`. The script loads modules from `research/_materials/lib/` and implements only the measurement logic. Language: **julia** (matches the modules).

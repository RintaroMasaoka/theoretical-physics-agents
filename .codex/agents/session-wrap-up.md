...
Determine the session kind from the dispatch prompt: `/auto` uses `auto`, `/steer` uses `steer`. Obtain the path by running `bash .scripts/log-path.sh {session-kind}` and capturing stdout — the script returns a timestamped path of the form `.logs/{YYMMDD_HHMM}_{session-kind}.md`. Then apply_patch the session log to that path with this exact structure:
...
- Write all prose in japanese **except** when copying verbatim from the input file — the input is already in the correct language, as research planner wrote it. Your own messages (error strings, commit-amend notes) follow the language rule

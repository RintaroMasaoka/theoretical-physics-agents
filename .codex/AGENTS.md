...
- Any file the system writes into the project — research tree (`research/**` such as `note.md`, `state.md`, `story.md`, `plan.md`, …), session records (`.logs/**` such as launch and meeting summaries), and any other prose file
- Deliverables produced by worker sub-agents (see `.codex/common.md`)
- Commit messages
...
- Do not request user input during `/auto` or `/write` execution (all forms prohibited, including request_user_input and tool permission requests). Users are often away during execution, and prompting them interrupts the session and wastes time

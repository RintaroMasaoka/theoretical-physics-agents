...
- Do not request user input during `/run` or `/write` execution (all forms prohibited, including request_user_input and tool permission requests). Users are often away during execution, and prompting them interrupts the session and wastes time

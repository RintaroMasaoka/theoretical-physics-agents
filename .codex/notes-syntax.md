...
`concepts/` contains atomic concept definitions; concept-checker may create entries, but other workers treat it as read-only. Research knowledge is stored in the research tree's `note.md` files (one per node), where **each principal claim carries both a derivation** (inline or cited — see `.codex/research-tree.md` § Scope of "derivation") **and a provenance tag**. Wiki-link syntax lets you follow cross-references; verification provenance tags let you assess the confidence level and review history of claims.

...
research/                 # Research tree — draft facts, state, strategy, graph
  ...
  {Branch Name}/          #   Research direction (Title Case with spaces)
    ...
    _materials/analyses/              #     (optional) Clean analyses; material support, not fact authority
      {slug}.md           #       Format: see .claude/research-tree.md
...
File formats (findings.md, guide.md, plan.md, backlog.md, state.md) are defined in `.claude/research-tree.md`. This section covers only files specific to `/auto` operations.
...
Compact, but always in the structured format required by `.claude/agents/research-planner.md` § `research/focus.md` Format. No frontmatter. Overwritten at each session end (see `session-lifecycle.md` for the handoff). Points to the current focus node and carries short-term context only.

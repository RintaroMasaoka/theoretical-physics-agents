...
**Compaction survival fallback.** If the resume hook (via the `SessionStart` hook in `.claude/settings.json`, script `.scripts/check-auto-resume.sh`) fails to re-inject the skill content, read `.claude/skills/auto/SKILL.md` directly with the `Read` tool and follow its Session Start from this step.
...
Agent(subagent_type="guide-writer", prompt="""
## Task

...
This phase file is kept only so old links do not break. The active session-end procedure lives in `session-lifecycle.md` § Session End, and the research planner-owned close-session packet format lives in `.claude/agents/research-planner.md` § Session-End Mode.

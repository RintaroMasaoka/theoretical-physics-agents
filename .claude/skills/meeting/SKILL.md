...
**Principle: present artifacts before asking for decisions.** Meetings are real-time review sessions; the user needs the actual artifact in front of them before approval has meaning. Load the minimal state needed to identify candidate `note.md` deliverables, present their contents verbatim, then ask for approval, revision, or direction. Use AskUserQuestion for questions (text output questions risk missed responses).

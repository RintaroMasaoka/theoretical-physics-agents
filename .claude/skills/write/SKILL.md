...
- `AskUserQuestion` is prohibited. Users are often away, and asking questions interrupts the session and wastes time
...
**Launch method:** Use the same Pattern A / Pattern B mechanics defined in `.claude/skills/auto/phases/dispatch.md`. Load that file when launching or waiting on subagents; reuse only its orchestration mechanics, not `/auto`'s research-cycle policy.

...
4. `.claude/naming.md`
5. `.claude/research-tree.md` § concepts/ and § conventions.md (for the concept/convention boundary)
...
- If it does not exist: propose a small, scoped note with the reuse-license front matter from `.claude/naming.md`. For general physics concepts, use standard field knowledge. For project-specific vocabulary, read only durable non-dot links from the target file and define the term as a reader aid, not as a project result
...
Each proposed concept note should be short and self-contained. Use the front matter from `.claude/naming.md`; then write whatever body prose makes the name clear. Typical body elements:

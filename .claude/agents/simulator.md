...
Write or extend a script under the correct `src/` per the canonical placement rule (usually the target node; higher up if the script is shared — see `.claude/research-tree.md` § Computation Artifacts). If extending an existing script (per §1), use Edit; if creating a new one, write `{slug}.{ext}` in that `src/`. The script loads modules from `research/lib/` and implements only the measurement logic. Language: **julia** (matches the modules).
...
Write a report-ready section in your `.logs/` simulation deliverable — a self-contained explanation of this simulation for the human researcher (the user). This is the material curator can promote into `reports/{slug}.md` after critic review and provenance closure. Write in **japanese**.

...
Determine the session kind from the dispatch prompt: `/auto` uses `auto`, `/steer` uses `steer`. Obtain the path by running `bash .scripts/log-path.sh {session-kind}` and capturing stdout — the script returns a timestamped path of the form `.logs/{YYMMDD_HHMM}_{session-kind}.md`. Then Write the session log to that path with this exact structure:

...
| `story.md` | Paper narrative surface (optional) | Paper storyline, presentation order, and narrative positioning. Write authority belongs to `/launch`, `/meeting`, and `/write`. It is not the parent-child routing map, not strategy, not fact authority, not `/auto` working memory, and not part of the `map.md` protocol |
...
| `concepts/{term}.md` | Naming / reader bridge | **Reusable-name license plus reader bridge.** Explains reusable handles and records what later readers may infer, carry, merge, or not merge from the name. Not fact authority, not a convention ledger, not workflow state. See `.claude/naming.md` |
...
`map.md` is not a fact authority and must not replace child facts. Established reusable facts and derivations live in `findings.md`. Working claims, unknowns, disputes, and absorbed evidence summaries live in `state.md`. `_materials/analyses/*.md` and `checks/` may carry inspectable support or verification, but they are not adopted fact authority by themselves. `map.md` may summarize a child's implication for the parent, but it should link to the child surface that carries the actual state or fact. It is also not `plan.md`: it should not say what to try next unless that line is framed as a read/reopen condition for a child. Strategy, method choice, and success criteria belong in `plan.md` or `research/focus.md`.
...
`story.md` is not part of this protocol. It belongs to paper-narrative authority, not parent-child routing authority. Do not use `story.md` as the parent-child routing map unless a future framework change explicitly redefines it.

## story.md — Paper Narrative Surface

`story.md` is the optional paper-narrative surface for storyline, presentation order, and narrative positioning. Its write authority belongs to `/launch`, `/meeting`, and `/write`: `/launch` may initialize or pivot the project/subtree narrative, `/meeting` may record human-approved narrative reframing, and `/write` may revise paper presentation when drafting exposes a better manuscript order.

`/auto` roles may read existing `story.md` as narrative context, but they do not create or update it during ordinary research cycles. `story.md` does not authorize facts, status, active work, child reopen conditions, worker dispatch, or parent-child routing. Use `map.md` for parent-child context routing and `plan.md` for strategy/decomposition. If `/auto` detects that `story.md` is stale or conflicts with current research memory, it should flag the issue for the next `/launch`, `/meeting`, or `/write` workflow rather than editing `story.md`.
...
**Maintenance rule.** `/launch` and `/meeting` perform the write-time routing check before adding a principle. Curator performs the maintenance check when touching `principles.md` and during session-end sweep. Entries that are really plan, source priority, notation, fact prose, or framework workflow rules should be moved to the right surface when the move is mechanical, or flagged for human/research-planner judgment when the scientific meaning would change. Entries that are really project thesis, narrative success condition, or paper storyline are flagged for `/launch`, `/meeting`, or `/write` because `story.md` is not a curator write surface.
...
## concepts/ — Reusable-Name Licenses and Reader Bridges

`concepts/` contains reusable-name licenses and reader bridges. A reusable name is a compact handle that has already been given handle shape in project memory; the concept note records the name's expansion, grounding, stability, carry scope, claim permission, merge boundaries, and reader action per `.claude/naming.md`. The prose body explains the name for a neighbouring-field researcher.

Concept notes are not project-fact authority, not a convention ledger, and not workflow state. A concept note may explain how to read a name, but it does not prove that the named construction exists, is unique, is important, or is equivalent to a source object. Those claims live in findings.md, `_materials/analyses/*.md`, and `checks/` according to their evidence contracts. Notation/sign/order/normalization-bound names belong in `conventions.md` or link there.

**Creation bias.** Catch reusable names when they are introduced or promoted, not during rare tree-wide audits. Worker submissions and `research/focus.md` may carry `Naming decisions`; curator closes those routed names when it absorbs transactions or promotes durable prose. Concept-checker and self-check are audit tools for suspected reader opacity, not the only way concept notes are born.

**Pollution control.** If a name is one-off, local, diagnostic-only, or misleading as shared vocabulary, do not create a concept note merely to tidy prose. Define it locally or replace it with plain prose. If the name becomes shared, write the concept note with reuse-license front matter so future agents know what the name does and does not license.

**Reading rule.** findings.md may link to concepts for reusable explanation, but findings.md must remain understandable at the level of its principal claim, scope, derivation skeleton, and limitation without requiring the concept file to carry hidden project substance. A concept link is a name-reading aid, not a substitute for claim derivation or verification.
...
**What findings.md is not.** It is not current research state, not a chronological log, not a map, not a plan, not a backlog, not a draft index, and not a stamp registry. A reader should not need `state.md`, `map.md`, `plan.md`, `backlog.md`, `.logs/`, or session memory to know what fact is being stated and why the node currently treats it as established or limited.
...
6. *Every non-common reusable name is gated* — for each reusable name or term that a reader in a neighbouring field would not immediately recognise, use one of these gates:
   ...
   - *Plain-prose replacement*: for local, nonce, or diagnostic labels that should not become shared vocabulary.
   
   When in doubt, define locally first. Create a concept note in `concepts/` when the name is reusable across nodes or its explanation would otherwise be repeated; concept notes are reusable-name licenses and reader bridges, not authority for project facts.
...
**Reading-route discipline.** Do not make `state.md` the default first read. `state.md` is working memory and evidence absorption; it can be long, chronological, and distracting by design. A guide should summarize the present oversight issue in ordinary prose, then point to the smallest surface that answers the next question: `findings.md` for reusable facts, `checks/` for verification, `_materials/analyses/` for a clean derivation or computation, `map.md` for child roles and reopen conditions, `plan.md` for strategy/decomposition, and `state.md` only when the human specifically needs current working-state details or evidence chronology.
...
A node's active decomposition and strategy document: structural dependencies and planner-supplied approach choices or success criteria. Parent-child reading roles belong first in `map.md`; plan.md records child roles only when they are part of strategy or decomposition rationale. Curator-maintained means graph consistency, not scientific authorship: research planner may direct strategy changes via `research/focus.md § Tree Directives`; curator rewrites plan.md when node creation, closure, archive, or reparenting makes recorded child/decomposition bookkeeping false, and may mark old strategy as stale/invalid when the contradiction is mechanical. Curator does not infer what to try next, which method to prioritize, or success criteria from structure; new route priority, method choice, or success criteria require a research-planner or user-present directive. Rewritten (not appended) when the active decomposition changes.

```markdown
{Free-form decomposition / strategy notes. No prescribed sections.
Typical content: decomposition rationale, why children exist when that is strategic,
...
**No template.** The content depends on the node's nature. A branch node might describe why its decomposition exists and which strategic choices constrain it; a leaf might outline an approach only when that approach is useful durable context rather than a transient task instruction. Do not use plan.md as the routine child synopsis; use map.md for that.

**Relationship to map.md and state.md**: map.md records how the parent reads children; plan.md records active decomposition and planner-supplied strategy; state.md captures current understanding and accumulated evidence. When child roles/status change, update map.md. When strategy changes, rewrite plan.md. When results come in, update state.md.
...
research/
  ...
  guide.md              (human oversight entrypoint)
  map.md               (optional parent-level child/context routing)
  ...
  Paradox Resolution/
    ...
    map.md             (optional child/context routing)
  ...
  Lattice BKT/
    ...
    map.md             (child roles, status, parent implications, reopen conditions)
...
| **Research tree — map** | `research/**/map.md` | Read-only | Curator context-routing maintenance from planner directives, presentation transactions, lifecycle changes, and absorbed evidence | Parent-level map of direct children: roles, status, parent implications, live-work state, and reopen/deep-read conditions |
| **Research tree — plan** | `research/**/plan.md` | Read-only | Curator graph-consistency maintenance from planner directives and absorbed evidence | Decomposition and planner-supplied strategy: approach decisions, success criteria, and strategic child roles |
...
| **Research story** | `research/**/story.md` | Read-only for `/auto` workers | `/launch`, `/meeting`, `/write` | Paper storyline, presentation order, and narrative positioning; not fact proof, verification, strategy, state, or child-routing surface |
